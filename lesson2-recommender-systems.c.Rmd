...
as.numeric(viewed_movies["149",]) # user 1's viewing history
as.numeric(viewed_movies["177",]) # user 2's viewing history
...
for(i in 1:14){
  for(j in (i+1):15){
    user_similarities[i,j] <- cosine_sim(as.numeric(viewed_movies[i,]), as.numeric(viewed_movies[j,]))
  }
}
...
# by movie
# cosine(viewed_movies)

# by user
c <- cosine(t(viewed_movies))
diag(c) <- 0
```
...
knn_ids <- c(names(sort(user_similarities["149",], decreasing = T))[1:5])
...
# intrain <- createDataPartition(as.numeric(rownames(viewed_movies)), p = 0.7, list = FALSE)
# training <- viewed_movies[intrain,]
# testing <- viewed_movies[-intrain,]
```
...
evaluate_fit_l2_bias2 <- function(x, observed_ratings, nrow, ncol, k, lambda, bias = TRUE){
...
startTime <- Sys.time()
  ...
  rec4 <- nlm(evaluate_fit_l2_bias2, observed_ratings = ratings_wide, lambda = 3e-3, nrow = 15, ncol = 20, k = 5, p = runif(220), iterlim = 1e4, steptol = 1e-2)
