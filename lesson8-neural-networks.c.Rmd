...
```
layer_dense(units = 32, # number of hidden neurons
            input_shape = c(201)) # number of input nodes
            %>% layer_activation('relu') # activation function
``` 
...
model %>% 
  ...
  layer_dense(units = 10, activation = 'softmax') # probability transformation - usefull for classifications
```
...
history <- model %>% fit(
  ...
  validation_split = 0.2 # split training into 80% internal training and 20% internal test
)
```
